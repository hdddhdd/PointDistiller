# python cal_flop_params_KITTI.py configs/second/hv_second_secfpn_6x8_80e_kitti-3d-3class.py --device cuda --show-unsupported --show-uncalled
...
DEFAULT_KITTI_CONFIG = (
    'configs/pointpillars/hv_pointpillars_secfpn_6x8_160e_kitti-3d-3class.py')
DEFAULT_KITTI_NUM_POINTS = 16000


def parse_args():
    parser = argparse.ArgumentParser(
        description='Compute FLOPs/params for KITTI 3D models in MMDetection3D')
    parser.add_argument(
        'config',
        nargs='?',
        default=DEFAULT_KITTI_CONFIG,
        help='KITTI config file path')
    parser.add_argument(
        '--num-points',
        type=int,
        default=DEFAULT_KITTI_NUM_POINTS,
...
def _infer_point_dim(cfg) -> int:
    ...
    candidate_roots = [data_cfg.get('test', {}), data_cfg.get('val', {})]
    for root in candidate_roots:
        pipeline = root.get('pipeline', [])
        if not pipeline and isinstance(root.get('dataset', None), dict):
            pipeline = root['dataset'].get('pipeline', [])
        if not pipeline:
            continue
