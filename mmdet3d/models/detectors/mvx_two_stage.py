...
# --------------------------------------------------------
# PointDistiller
# Copyright (c) Runpei Dong & Linfeng Zhang
...
class MVXTwoStageDetector(Base3DDetector):
    ...
    def extract_pts_feat(self, pts, img_feats, img_metas,get_info=False):
        ...
        voxel_features = self.pts_voxel_encoder(voxels, num_points, coors,
                                                img_feats, img_metas)
        info['voxel_features'] = voxel_features
        info['pts'] = pts
        batch_size = coors[-1, 0] + 1
        x = self.pts_middle_encoder(voxel_features, coors, batch_size)
        info['num_points'] = num_points
        x = self.pts_backbone(x)
        info['backbone_feature'] = x
    ...
    def extract_feat(self, points, img, img_metas,get_info=False):
        """Extract features from images and points."""
        img_feats = self.extract_img_feat(img, img_metas)
        if get_info:
            pts_feats, info = self.extract_pts_feat(points, img_feats, img_metas, get_info=True)
            return img_feats, pts_feats, info
        else:
    ...
    def get_teacher_info(self,
        ...
        img_feats, pts_feats, t_info = self.extract_feat(
            points, img=img, img_metas=img_metas, get_info=True)
        return t_info
    ...
    def forward_train(self,
        ...
        """
        def get_knn_voxel_backbone(teacher_feature, student_feature, num_voxel_points=None,
            ...
            reweight = index_feature(num_voxel_points.unsqueeze(-1), query_voxel_idx) #1 x 256 x 1 
            reweight = reweight.unsqueeze(0)
        ...
        # main function
        img_feats, pts_feats, student_info = self.extract_feat(
            points, img=img, img_metas=img_metas, get_info=True)
        losses = dict()
        ...
        if teacher_info is not None:
            ...
            kd_loss = get_knn_voxel_backbone(
                    teacher_feature=teacher_info['neck_feature'][0], student_feature=student_info['neck_feature'][0],
                    num_voxel_points=None, num_voxels=256, kneighbours=128,
